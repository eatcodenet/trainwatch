...
cp ${src_dir}/tw-train-movement/build/libs/tw-train-movement*.jar ${aws_build_dir}/libs
...
echo "Pushing to S3"
bundle_name="TrainWatchBackendLatestBundle.zip"
aws_result=$(aws --profile eatcode deploy push --source ${aws_build_dir} --application-name TrainWatchBackend --ignore-hidden-files --s3-location s3://eatcode-trainwatch-deploy/${bundle_name})
etag=$(sed 's/.*eTag="\(.*\)".*/\1/g' <<< ${aws_result})

echo "Deploying revision..."
aws --profile eatcode deploy create-deployment --application-name TrainWatchBackend --s3-location bucket=eatcode-trainwatch-deploy,key=${bundle_name},bundleType=zip,eTag=\"${etag}\" --deployment-group TrainWatch
echo "To deploy same revision again run:"
echo "aws --profile eatcode deploy create-deployment --application-name TrainWatchBackend --s3-location bucket=eatcode-trainwatch-deploy,key=${bundle_name},bundleType=zip,eTag=\"${etag}\" --deployment-group TrainWatch"
echo
