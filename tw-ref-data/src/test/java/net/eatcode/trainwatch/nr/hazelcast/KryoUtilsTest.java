...
public class KryoUtilsTest {
    ...
    @Test
    public void serializesLocalTimeInHz() {
        Schedule s1 = new Schedule();
        s1.id = "schedule id";
        s1.arrival = LocalTime.of(6, 30);
        s1.departure = LocalTime.of(7, 30);

        HazelcastInstance client = new HzClientBuilder().build("localhost");
        IMap<String, byte[]> map = client.getMap("schedule2");
        map.put("s1", KryoUtils.toByteArray(s1));
        map.put("s2", KryoUtils.toByteArray(s1));
        Schedule s2 = KryoUtils.fromByteArray(map.get("s1"), Schedule.class);
        client.shutdown();
        assertThat(s2.id, is("schedule id"));
        assertThat(s2.arrival, is(LocalTime.of(6, 30)));
    }
